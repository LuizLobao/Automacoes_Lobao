...
def puxa_deflac_ref():
	...
	try:
		...
		df.to_csv(caminho_arquivo_csv, sep=';',header = True, index = False, mode = 'w')
...
def inserir_dados_tendencia(conexao):
	...
	try:
		...
		logging.error('Erro ao inserir dados em TBL_CDO_APOIO_TENDENCIA - funcao: inserir_dados_tendencia')

def exportar_dados_tendencia_para_csv(conexao):
	try:
		comando_sql = '''select * from dbo.TBL_CDO_APOIO_TENDENCIA order by DATA'''
		...
		caminho_arquivo_csv = os.path.join(diretorio_temp, f'tendencia_{AAAAMMDD_referencia}.csv')
		df.to_csv(caminho_arquivo_csv, sep=';', header=True, index=False, mode='w')

		logging.info(f'Arquivo de tendência CSV criado em {caminho_arquivo_csv}')
	except Exception as e:
		logging.error(f'Erro ao exportar dados de tendência para CSV: {str(e)}')
